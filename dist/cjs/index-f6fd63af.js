...
BcCode.label = 'Code'; // BcCode.icon = '<svg viewBox="0 0 24 24"><path d="M9.4 16.6L4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0l4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z"/></svg>';
...
BcCode.doRender = function (options, preview) {
  const code = options.code; //hljs.highlight(options.language, options.code).value;

  const margin = preview ? '' : 'padding: 0 2rem'; //'margin: -2rem;' : 'padding: 0 2rem';

  const colors = options.theme == 'Dark' ? 'background: #353030; color: white;' : 'background: #f5f5f5;';
  console.log("Highlighter: ", hljs__default['default']);
  return `
